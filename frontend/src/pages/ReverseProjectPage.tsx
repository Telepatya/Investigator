      ...
      {view === "workspace" && <>
        ...
        <Section title="Static analysis">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {status.data?.can_resume && !status.data.can_recover_report && status.data.status !== "awaiting_turn_approval" && <button className="btn-primary" disabled={action.isPending || !health.data?.image_available} onClick={() => action.mutate("resume")}><Play size={15} /> Resume</button>}
          ...
          {status.data?.run && <div className="mt-4 rounded-2xl bg-[rgb(var(--panel-strong)/0.55)] p-4 text-sm"><div className="flex flex-wrap justify-between gap-2"><span className="font-semibold capitalize text-ink-100">{status.data.run.status.replace(/_/g, " ")}</span><span className="text-ink-400">Turns {status.data.run.turns_used} / {status.data.run.max_turns}</span></div><div className="mt-2 text-xs text-ink-400">{status.data.run.provider} · {status.data.run.model}</div>{status.data.run.error && <div className="mt-3 text-sev-critical">{status.data.run.error}</div>}</div>}
          {status.data?.status === "awaiting_turn_approval" && <div className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-300"><div className="font-semibold">More investigation turns requested</div><p className="mt-1">{status.data.run?.awaiting_reason}</p><div className="mt-3 flex gap-2"><button className="btn-primary" onClick={() => action.mutate("approve")}>Allow more turns</button><button className="btn-ghost" onClick={() => action.mutate("deny")}>Finish report now</button></div></div>}
          {action.error && <div className="mt-3 text-sm text-sev-critical">{String(action.error)}</div>}
        </Section>
      </>}

      {view === "report" && <div className="space-y-4">
        <Section title="Analysis report" right={report.data && <div className="flex flex-wrap items-center gap-2"><span className={`chip ${report.data.signature_status === "signed" ? "bg-emerald-400/10 text-emerald-400" : "bg-amber-400/10 text-amber-300"}`}><ShieldCheck size={12} /> {report.data.signature_status}</span>{report.data.signature_status !== "signed" && <button className="btn-ghost" disabled={reportSign.isPending} onClick={() => reportSign.mutate()}><ShieldCheck size={14} /> Retry signing</button>}<button className="btn-ghost" disabled={iocs.isPending} onClick={() => iocs.mutate()}><RefreshCw size={14} /> Regenerate IOCs</button></div>}>{report.isLoading ? <Spinner label="Loading report…" /> : report.data ? <><ReverseMarkdown content={report.data.content} />{report.data.signature_error && <div className="mt-4 rounded-xl border border-amber-400/30 bg-amber-400/10 p-3 text-xs text-amber-300">The report is safely stored but signing needs attention: {report.data.signature_error}</div>}{reportSign.error && <div className="mt-3 text-sm text-sev-critical">{String(reportSign.error)}</div>}</> : <div className="py-12 text-center text-sm text-ink-400">Complete an analysis to generate a report.</div>}</Section>
        {report.data && <Section title="Independent evidence verification" right={<span className={`chip ${report.data.verification_status === "verified" ? "bg-emerald-400/10 text-emerald-400" : "bg-amber-400/10 text-amber-300"}`}><CheckCircle2 size={12} /> {report.data.verification_status.replace(/_/g, " ")}</span>}><div className={`rounded-2xl border p-4 text-sm ${report.data.verification_status === "verified" ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-amber-400/25 bg-amber-400/10 text-amber-200"}`}><p>{report.data.verification_summary || "No verifier summary is available."}</p>{report.data.verification_error && <p className="mt-2 text-xs">{report.data.verification_error}</p>}</div>{Object.keys(report.data.verification_details).length > 0 && <details className="mt-3 rounded-xl bg-[rgb(var(--panel-strong)/0.55)] p-3"><summary className="cursor-pointer text-xs font-semibold text-ink-300">Verifier findings and revision decision</summary><div className="mt-3"><CodeBlock>{JSON.stringify(report.data.verification_details, null, 2)}</CodeBlock></div></details>}{report.data.verification_status !== "verified" && <button className="btn-ghost mt-3" disabled={reportVerify.isPending} onClick={() => reportVerify.mutate()}><RefreshCw size={14} /> Retry verification</button>}{reportVerify.error && <div className="mt-3 text-sm text-sev-critical">{String(reportVerify.error)}</div>}</Section>}
