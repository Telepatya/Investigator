...
def _extract_report_section(text: str, section_name: str) -> str:
    """Use ForensicBuddy's permissive section extraction behavior."""
    patterns = (
...
class ReverseAnalysisManager:
    ...
    def _system_prompt(
        ...
        """Faithful port of ForensicBuddy's original malware-analysis prompt."""
...
Keep your report concise but technically rigorous."""
        ...
        try:
            ...
            while True:
                ...
                if call.tool not in _approved_tools(project_id):
                    ...
                    no_progress_turns = 0
                # Preserve ForensicBuddy's conversation shape: tool results are
                # returned as a one-entry array matching its one-tool-per-turn loop.
    ...
    async def _finalize(self, project_id: str, run_id: str, report: str) -> None:
        ...
        try:
            ...
            logger.warning("ForensicBuddy IOC inventory step failed for %s", project_id, exc_info=True)
